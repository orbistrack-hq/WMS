  ...
  function sync() {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) setError(res.error)
        ...
        setNote(
          `Synced ${res.products} product${res.products === 1 ? "" : "s"}: ${res.created} new, ${res.updated} updated${res.skipped ? `, ${res.skipped} skipped` : ""}. Stock seeded on ${res.stockSynced} new SKU${res.stockSynced === 1 ? "" : "s"}, cost updated on ${res.costSeeded}.` +
