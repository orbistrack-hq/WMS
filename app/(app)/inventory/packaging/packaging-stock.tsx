...
export function PackagingStock({
  ...
  types: StockType[]
  levels: StockLevel[]
  // Writes are internal-ops only (admin/operator). Non-ops still see the counts.
  canManage: boolean
}) {
  ...
  const byType = new Map(levels.map((l) => [l.packaging_type_id, l]))
    ...
    <div className="flex flex-col gap-3">
      ...
      {types.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          No active packaging types yet. Add them in Settings → Packaging.
        </p>
      ) : (
        <ul className="flex flex-col divide-y divide-border rounded-lg border border-border">
          {types.map((t) => {
            const lvl = byType.get(t.id)
      ...
      <p className="text-xs text-muted-foreground">
        One central pool per type, shared across every site. Packaging is consumed
        automatically when an order is packed (counted once per combined-order
        group). Receiving adds stock; an adjustment is a counted correction. Stock
        can read negative if more was used than received — receive or adjust to
        reconcile.
      </p>
    </div>
  )
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-1">
          ...
          <Button
            ...
            onClick={() =>
              run(() => receivePackaging(typeId, Number(recvQty), recvNote))
            }
          >
            <PackagePlus />
          </Button>
        ...
        <div className="flex gap-1">
          ...
          <Button
            ...
            onClick={() => {
              ...
              run(() => adjustPackaging(typeId, Number(adjDelta), adjNote))
            }}
          >
            <Check />
          </Button>
