...
- **Remaining:**
  - **Slice 2b (deferred) — wave (batch) screen** still uses the old single-threshold seed (functional, feeds its own confirm). Rework to the same config-driven per-type auto-record is the last FB-6 follow-up; the per-order pack screen it links to is already correct.
  - `tsc` not runnable in-sandbox → `pnpm typecheck && pnpm test` before merge.
