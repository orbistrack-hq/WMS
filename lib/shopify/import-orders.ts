  ...
  if (order.lifecycle === "fulfilled") {
    if (storeAutoFulfillEnabled()) {
      const { error } = await client.rpc("fulfill_order", {
        ...
        // Store completed it upstream before/without local packing — mark it so
        // it's distinguishable from a local pack and surfaces in the packaging-gap
        // report for after-the-fact cost capture.
  ...
  if (status === "fulfilled" || status === "cancelled") {
    ...
    if (status === "cancelled" && order.lifecycle === "fulfilled") {
      await flagShipConflict(
        client,
        wmsOrderId,
        `Shopify reported this order shipped/completed on ${
