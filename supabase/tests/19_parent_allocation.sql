-- Parent bulk inventory + allocation schema (migration 0028).
-- Covers: UoM conversion, child weight-variant uniqueness, parent level
-- primitives + ledger, and the non-negative on_hand guard.
-- Uses seeded product 33333333-...0001 (Wildflower Honey) at site
-- 11111111-1111-1111-1111-111111111111 (which already has one non-weight child).
begin;
select plan(18);
...
-- ---- child weight-variant uniqueness --------------------------------------
select lives_ok(
...
-- ---- parent inventory primitives ------------------------------------------
select lives_ok(
  $$ select _parent_inv_lock('33333333-0000-0000-0000-000000000001',
                             '11111111-1111-1111-1111-111111111111') $$,
  '_parent_inv_lock materializes a zero row on demand' );
select is(
  (select on_hand_grams from parent_inventory where product_id=:PROD and site_id=:SITE),
  0::numeric, 'new parent level starts at 0 g' );

select lives_ok(
  $$ select _parent_inv_write('33333333-0000-0000-0000-000000000001',
       '11111111-1111-1111-1111-111111111111', 448, 0,
       'intake','manual',null,'LOT-1','test intake') $$,
  'intake credits 448 g' );
select is(
  (select on_hand_grams from parent_inventory where product_id=:PROD and site_id=:SITE),
  448::numeric, 'parent on_hand = 448 g after intake' );
select is(
  (select count(*)::int from parent_inventory_ledger
     where product_id=:PROD and site_id=:SITE and reason='intake'),
  1, 'one intake ledger row written' );

-- allocation-style debit: remove 56 g, bump allocated for reporting.
select lives_ok(
  $$ select _parent_inv_write('33333333-0000-0000-0000-000000000001',
       '11111111-1111-1111-1111-111111111111', -56, 56,
       'allocation','allocation',null,null,'test alloc') $$,
  'allocation debit of 56 g succeeds' );
select is(
  (select on_hand_grams from parent_inventory where product_id=:PROD and site_id=:SITE),
  392::numeric, 'parent on_hand = 392 g after 56 g allocated' );
select is(
  (select allocated_grams from parent_inventory where product_id=:PROD and site_id=:SITE),
  56::numeric, 'allocated_grams = 56 g (reporting counter)' );

-- non-negative guard: cannot debit more than on hand.
select throws_ok(
  $$ select _parent_inv_write('33333333-0000-0000-0000-000000000001',
       '11111111-1111-1111-1111-111111111111', -500, 500,
