-- Intake + allocation RPCs (migration 0029).
-- Covers: intake grams credit, cross-site allocation, the over-allocation guard,
-- null-grams rejection, allocation history, and idempotent replay.
begin;
select plan(17);
...
create temp table _in on commit drop as
  select public.intake_receive(:PROD1, :MAIN, 1, 'lb', 'LOT-A', 'first pound') as r;
select is( ((select r->>'on_hand_grams' from _in))::numeric, 448::numeric,
  'intake of 1 lb credits 448 g to the parent pool' );
select is(
  (select on_hand_grams from parent_inventory where product_id=:PROD1 and site_id=:MAIN),
  448::numeric, 'parent_inventory on_hand = 448 g after intake' );
select is(
  (select count(*)::int from parent_inventory_ledger
     where product_id=:PROD1 and site_id=:MAIN and reason='intake'),
...
select is(
  (select on_hand_grams from parent_inventory where product_id=:PROD1 and site_id=:MAIN),
  308::numeric, 'parent pool debited to 308 g' );
select is(
  (select allocated_grams from parent_inventory where product_id=:PROD1 and site_id=:MAIN),
...
select is( (select on_hand from inventory_levels where child_sku_id=:B3), 8,
  'EAST 3.5g child credited 8 units (cross-site)' );
select is(
...
-- ---- idempotent replay (separate pool) ------------------------------------
select public.intake_receive(:PROD2, :MAIN, 100, 'g');
...
select is(
  (select on_hand_grams from parent_inventory where product_id=:PROD2 and site_id=:MAIN),
