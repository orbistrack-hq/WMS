...
declare v_uid uuid;
begin
  select id into v_uid from auth.users where email = 'jay@packsglobal.com';
...
--   where id = (select id from auth.users where email = 'packer@packsglobal.com');
...
-- declare v_uid uuid;
-- begin
--   select id into v_uid from auth.users where email = 'client@example.com';
